...
var	vstrings; try { vstrings = parent.ary_strings; } catch(e) { vstrings = undefined; }
function DictionaryStrings()
{
	this.dicObj = {};
	
	if(typeof(vstrings) == 'undefined' || !vstrings)
	{
		//alert("Load strings failure for js!!"); //foxconn steve delete
		return;
	}
	for (var i=0; vstrings[i] && vstrings[i][0] != ''; i++)
	{
...
function chineseCheck(object, objectId)
{
	...
	var objName = obj ? (obj.innerText || obj.textContent) : "Warning : input";
	var inputStr = object ? object.value : "";
	var i;
