...
function handle_tracking(array $segments, string $method): void
{
    ...
    if ($sub === 'timer' && $method === 'POST') {
        ...
        try {
            ...
            if ($technicianId) {
                $techStmt = $pdo->prepare('SELECT technician_id, email, full_name FROM technicians WHERE technician_id = :id');
                $techStmt->execute(['id' => $technicianId]);
                $tech = $techStmt->fetch();
            }
            if (!$tech && $email) {
                $emailStmt = $pdo->prepare('SELECT technician_id, email, full_name FROM technicians WHERE email = :email');
                $emailStmt->execute(['email' => normalize_email($email)]);
                $tech = $emailStmt->fetch();
            }

            if ($labId) {
                $labStmt = $pdo->prepare(
                    'SELECT l.lab_id, l.lab_name, d.model, d.device_name
                     FROM labs l JOIN devices d ON d.device_id = l.device_id
                     WHERE l.lab_id = :id'
                );
                $labStmt->execute(['id' => $labId]);
                $lab = $labStmt->fetch();
            }

            if ($tech && $lab) {
                $timer['technician_id'] = (string)$tech['technician_id'];
                $timer['name'] = (string)($tech['full_name'] ?? $timer['name']);
                $timer['email'] = (string)($tech['email'] ?? $timer['email']);
                $timer['device'] = (string)($lab['model'] ?? $lab['device_name'] ?? $timer['device']);
                $timer['device_model'] = $timer['device'];
                $timer['lab_name'] = (string)($lab['lab_name'] ?? $labId);

                $insert = $pdo->prepare(
                    'INSERT INTO sessions (session_id, technician_id, lab_id, started_at, finished_at, duration_sec, mode, created_at, updated_at)
                     VALUES (:session_id, :technician_id, :lab_id, :started_at, :finished_at, :duration_sec, :mode, NOW(), NOW())
                ...
                $timer['session_id'] = $savedRow ? (string)$savedRow['session_id'] : $timer['session_id'];
                $timer['saved'] = true;
            }
        } catch (Throwable) {
            $timer['saved'] = false;
