...
--
-- NOTE: UPDATE statements that JOIN class_enrollments / training_classes are
-- guarded with IF EXISTS because migration 025 drops those tables.
-- The column additions and index creations are always safe.
...
-- 2. Backfill class_code from class_enrollments & training_classes (only if tables exist)
DO $$
BEGIN
    ...
    AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'training_classes') THEN
        ...
        -- Backfill dashboard_region from training_classes
        UPDATE users u
...
-- 3. Backfill dashboard_region from class_code prefix patterns
UPDATE users
...
-- 4. Backfill region_id, region_code, and unit_name from regions catalog
UPDATE users u
...
-- 5. Ensure fallback for unit_name if still NULL
UPDATE users
...
-- 6. Performance indexes for direct zero-join queries
