...
{
    if (!$userId || ($timer['lab_id'] ?? '') === '') {
        return false;
    }

    $existing = $pdo->prepare('SELECT attempt_id FROM lab_attempts WHERE source_timer_session_id = :timer_id');
    $existing->execute(['timer_id' => $timerId]);
    if ($existing->fetchColumn()) {
        return true;
    }

    $normalizedMode = ($timer['mode'] ?? '') === 'Hướng dẫn' ? 'guide' : 'practice';
    $assignment = $pdo->prepare(
        ...
        SELECT assignment.assignment_id,
         ...
         WHERE enrollment.user_id = :user_id
           ...
           AND assignment.assigned_at <= CAST(:finished_at_3 AS timestamptz)
           AND enrollment.status = 'active'
           AND enrollment.valid_from <= CAST(:finished_at AS timestamptz)::date
           AND (enrollment.valid_to IS NULL OR enrollment.valid_to >= CAST(:finished_at_2 AS timestamptz)::date)
           AND training.status = 'active'
         ORDER BY training.start_date DESC, assignment.assigned_at DESC
         LIMIT 1
    ...
    $assignment->execute([
        ...
        'finished_at_2' => $timer['finished_at'],
        'finished_at_3' => $timer['finished_at'],
    ]);
    $assignmentRow = $assignment->fetch();
    $assignmentId = is_array($assignmentRow) ? (string)($assignmentRow['assignment_id'] ?? '') : '';
    if ($assignmentId === '') {
        return false;
    }

    $attemptNumberStatement = $pdo->prepare(
        'SELECT COALESCE(MAX(attempt_no), 0) + 1 FROM lab_attempts WHERE assignment_id = :assignment_id AND mode = :mode'
    );
    $attemptNumberStatement->execute(['assignment_id' => $assignmentId, 'mode' => $normalizedMode]);
    $attemptNumber = (int)$attemptNumberStatement->fetchColumn();

    $status = (string)$timer['status'];
    $outcome = match ($status) {
        'failed' => 'failed',
        'completed' => ($timer['is_passed'] ?? null) === true ? 'passed' : null,
        default => null,
    };
    $details = is_array($timer['grading_details'] ?? null) ? $timer['grading_details'] : [];
    $errorCount = count(array_filter(
        $details,
        static fn(mixed $detail): bool => is_array($detail)
            && array_key_exists('passed', $detail)
            && $detail['passed'] === false
    ));
    ...
    $insertAttempt = $pdo->prepare(
        ...
        INSERT INTO lab_attempts (
            ...
            :started_at, :finished_at, :duration_seconds, :score, :error_count,
            :last_action, :source_event_key, :source_timer_session_id,
            FALSE, CAST(:metadata AS jsonb), NOW(), NOW()
        )
        ON CONFLICT (source_timer_session_id) DO NOTHING
        SQL
    );
    $insertAttempt->execute([
        ...
        'finished_at' => $timer['finished_at'],
        'duration_seconds' => $timer['duration_sec'],
        'score' => $timer['score'],
        'error_count' => $errorCount,
        'last_action' => $timer['last_action'],
        'source_event_key' => 'tracking:' . $timer['submission_id'],
        'source_timer_session_id' => $timerId,
        'metadata' => $metadata,
    ]);

    if ($normalizedMode === 'practice') {
        if ($status === 'completed' && $outcome === 'passed') {
            $reportedFirstTry = $timer['completed_first_try'] ?? null;
            $firstPassAttemptNumber = $reportedFirstTry === true
                ? 1
                : ($reportedFirstTry === false ? max(2, $attemptNumber) : $attemptNumber);
            $firstTryEvidence = is_bool($reportedFirstTry) ? 'legacy_reported' : 'unknown';
            $update = $pdo->prepare(
                <<<'SQL'
                UPDATE lab_assignments
                   SET status = 'passed',
                       completed_at = COALESCE(completed_at, CAST(:finished_at AS timestamptz)),
                       passed_at = COALESCE(passed_at, CAST(:passed_at AS timestamptz)),
                       first_pass_attempt_no = COALESCE(first_pass_attempt_no, :first_pass_attempt_no),
                       first_try_evidence = CASE
                           WHEN status = 'passed' THEN first_try_evidence
                           ELSE :first_try_evidence
                       END,
                       updated_at = NOW()
            ...
            $update->execute([
                'finished_at' => $timer['finished_at'],
                'passed_at' => $timer['finished_at'],
                'first_pass_attempt_no' => $firstPassAttemptNumber,
                'first_try_evidence' => $firstTryEvidence,
                'assignment_id' => $assignmentId,
            ]);
            ...
            $update = $pdo->prepare(
                <<<'SQL'
                UPDATE lab_assignments
                   SET status = CASE WHEN status = 'passed' THEN 'passed' ELSE 'completed' END,
                       completed_at = COALESCE(completed_at, CAST(:finished_at AS timestamptz)),
                       updated_at = NOW()
                 WHERE assignment_id = :assignment_id
                SQL
            );
            $update->execute(['finished_at' => $timer['finished_at'], 'assignment_id' => $assignmentId]);
        } elseif (in_array($status, ['failed', 'abandoned'], true)) {
            $update = $pdo->prepare(
                "UPDATE lab_assignments
                    SET status = CASE WHEN status IN ('passed', 'completed') THEN status ELSE 'in_progress' END,
                        updated_at = NOW()
                  WHERE assignment_id = :assignment_id"
            );
            $update->execute(['assignment_id' => $assignmentId]);
        }
    }
