...
var reboot_time_en = "0";
var reboot_time = "00:00";
var mon_val = "0";
var tue_val = "0";
var wed_val = "0";
var thu_val = "0";
...
function HiddenRebootTimer(){
        if(document.tool_reboottimer.reboottimer_enable[0].checked)
...
function Timenumber(){
        var timenum=document.tool_reboottimer.time.value;
...
function uiSave(){
        ...
        var mon = document.tool_reboottimer.mon;
        var tue = document.tool_reboottimer.tue;
        var wed = document.tool_reboottimer.wed;
        var thu = document.tool_reboottimer.thu;
        var fri = document.tool_reboottimer.fri;
        var sat = document.tool_reboottimer.sat;
        var sun = document.tool_reboottimer.sun;
        if(mon.checked)
                document.tool_reboottimer.mon_value.value=1;
        else
                document.tool_reboottimer.mon_value.value=0;

        if(tue.checked)
                document.tool_reboottimer.tue_value.value=1;
        else
                document.tool_reboottimer.tue_value.value=0;

        if(wed.checked)
                document.tool_reboottimer.wed_value.value=1;
        else
                document.tool_reboottimer.wed_value.value=0;

        if(thu.checked)
                document.tool_reboottimer.thu_value.value=1;
        else
                document.tool_reboottimer.thu_value.value=0;

        if(fri.checked)
                document.tool_reboottimer.fri_value.value=1;
        else
                document.tool_reboottimer.fri_value.value=0;

        if(sat.checked)
                document.tool_reboottimer.sat_value.value=1;
        else
                document.tool_reboottimer.sat_value.value=0;

        if(sun.checked)
                document.tool_reboottimer.sun_value.value=1;
        else
                document.tool_reboottimer.sun_value.value=0;
        ...
        showSpin();
        document.tool_reboottimer.saveFlag.value=1;
        document.tool_reboottimer.submit();
}

</script>
        </head>

        <body onLoad="HiddenRebootTimer()" style="background:#4acbd6;">
                <FORM METHOD="POST" ACTION="/cgi-bin/tools_reboottimer.asp" name="tool_reboottimer">
                        <div id="pagestyle">
                                <div id="contenttype">  
                                        <div id="block1" class="main_item">
                                                ...
                                                <table width="640" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="table-layout: fixed">
                                                        <tr height="30px">
                                                                <td align=left class="tabdata" style="width:250px;padding-left:20px;">Reboot Timer Status</td>
                                                                <td align=left class="tabdata">
                                                                        <INPUT TYPE="radio" NAME="reboottimer_enable" VALUE="1"  onClick="HiddenRebootTimer();" > Enable&nbsp;&nbsp;&nbsp;&nbsp;         
                                                                        <INPUT TYPE="radio" NAME="reboottimer_enable" VALUE="0"  onClick="HiddenRebootTimer();" > Disable 
																		<script>
																			if( reboot_time_en != "1")
																			{
																				document.getElementsByName("reboottimer_enable")[0].checked = false;
																				document.getElementsByName("reboottimer_enable")[1].checked = true;
																			}
																			else
																			{
																				document.getElementsByName("reboottimer_enable")[0].checked = true;
																				document.getElementsByName("reboottimer_enable")[1].checked = false;
																			}
																		</script>
																</td>
                                                        </tr>
                                        ...
                                        <div id="block1">
                                                ...
                                                <table width="640" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="table-layout: fixed">
                                                        <tr height="30px">
                                                                <td align=left class="tabdata" style="width:250px;padding-left:20px;">Reboot Time</td>
                                                                <td align=left class="tabdata">
                                                                        <INPUT TYPE="text" NAME="time" id="time" SIZE="7" MAXLENGTH="5"  VALUE="" >    (hour:min) 
																<script>
																	if( reboot_time != "N/A")
																	{
																		document.getElementById("time").value = reboot_time;
																	}
																</script>
                                                                </td>
                                                        </tr>
