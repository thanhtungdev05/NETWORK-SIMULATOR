...
{var data=[];var objValue={"band":"5G","index":1,"enable":1,"mode":mode,"pin":pincode};data.push(objValue);var ubusparam=new Array("rtweb.wifi","wlanWpsSet",{"bss":data});var jsonparam=[{"id":1,"params":ubusparam}];sk_auth_post(jsonparam,function(result){loading_show();setTimeout("hide_mode()",5000);});}
function setwpsCfg_en(enable)
{var data=[];var objValue={"band":"5G","index":1,"enable":enable};data.push(objValue);var ubusparam=new Array("rtweb.wifi","wlanWpsSet",{"bss":data});var jsonparam=[{"id":1,"params":ubusparam}];sk_auth_post(jsonparam,function(result){loading_show();setTimeout("hide_mode()",5000);});}
function BntClick_PbcShow()
{var toggleElement=document.getElementById('wps_hide');if(toggleElement.style.display==='none'){toggleElement.style.display='block';document.getElementById('wps_switch').value=(_('wlanMapSetting5g_disabled'));setwpsCfg_en(1);}else{toggleElement.style.display='none';document.getElementById('wps_switch').value=(_('wlanMapSetting5g_enabled'));setwpsCfg_en(0);}}
function BntClick_PbcApply()
{setwpsCfg("pbc","");}
function BntClick_PinApply()
{var pin=getValue("Txt_WPSPIN");if(isvalidpin(pin)==false)
{warninfo_show(_('wlanWpsConfig5g_pininvalid'));return false;}
setwpsCfg("pin",pin);}
